...
of PR #7 at its reviewed head `a086dc1d7c525f7329e250c68c2a68c092e25ad8`
(merge tree identical to it). Everything below this section that says
"this branch", "draft PR #7" or "production stays `b24d1f4`" is history.

Release record (both runs verified; the second independently by Astra):

| run | id | when (UTC) | map request | page |
|---|---|---|---|---|
| #23 schedule | 36074980438 | 09-24 23:53, deployed 23:54 | none — `RECOVERY NEEDED — no request ledger` | DEGRADED, designation moves WITHHELD, built 23:54Z |
| #24 schedule | 36095609134 | 09-25 04:43, deployed 04:44 | none — `RECOVERY NEEDED` | Game Day week 3 built 04:44:07Z, HTTP 200 |

The last map request was old-code run #22 (36037741578) at 2026-09-24
17:57:10Z. **Pending: the one bootstrap, no earlier than 2026-09-25
17:57:10Z**, and the first ordinary run after it. Until then the page says
RECOVERY NEEDED / PAUSED, which is correct.

**Bootstrap, for Astra (after 17:57:10Z and the log check in step 1 of
"Player-map bootstrap" below):**

    gh workflow run dashboard-artifact.yml --repo Kejjeh/gridiron --ref main -f player_map_bootstrap=true

(uses her own `gh` login; no token goes in the command). Pass when: that
run's log (step "Pull the week's nflverse frames and the player dump") has
exactly ONE `sleeper_players: N players` line and no `bootstrap refused`;
the next run of the workflow logs `sleeper_players: not requested — last
request …` (or, after a lost cache save, `… not shown to come from the
previous run …`; also no request); the Board's Designations date is the bootstrap run's time and
the RECOVERY NEEDED / PAUSED note is gone. **If the bootstrap run fails or
its result is unclear, do not dispatch again.** Read its log first. A
`sleeper_players: FAILED` line means a request WAS made (it counts: it is
ledgered before the GET), and so does a pull step that started and died
without any `sleeper_players:` line; a lost cache save can hide either from
the next bootstrap, so the next attempt waits 24 h after that run. Only a
run whose pull step never started (checkout, install, the sync step, runner
loss) made no request. Never use "Re-run" on it: a re-run is refused, and it
is not a way round the budget.

**Why builds arrive ~5 h apart** (`docs/review/schedule/`,
`schedule-gaps-2026-09-25.txt`, regenerated by `schedule_gaps.py`):
every scheduled run of both workflows ran once, first attempt, success; none
cancelled, queued or overlapping; runners in 1-3 s. Main's dashboard cron
was hourly (`41 * * * *`) until the merge, 15-minute only since. Most slots
produced no run at all — dashboard 19 runs / 92 hourly slots, then 2 / 43
quarter-hour slots; sync 30 / 131 — so quadrupling the slots added nothing.
GitHub documents that scheduled events are delayed under load and "may be
dropped"; it records no dropped event, so the internal cause is UNKNOWN and
nothing here promises a build interval. Observed gaps: 2.0-7.8 h, median
about 5 h. The pages claim no cadence: each input and the build carry their
own time, and the gates age from those.

One option within existing infrastructure, **proposed, not implemented**:
add `workflow_run: {workflows: ["Sleeper cloud sync"], types: [completed],
branches: [main]}` to the dashboard workflow, so each delivered sync event
also builds. Replayed on the observed times it halves the median gap
(5.1 h → 2.1 h) but not the worst (7.5 h → 7.2 h). Before adopting: the
workflow's `if:`/input handling must accept the new event, the run-number
carry chain and the 24 h budget are unchanged (same workflow, same
concurrency group), and it adds up to ~30 runs/day of hosted minutes on a
public repository. More cron slots are not an option: the evidence shows
they do not add runs. An external scheduler stays out of scope.

**Copy fix on `claude/schedule-gap-evidence` (draft, not merged):** during
RECOVERY NEEDED the carried map read "STALE (refresh FAILED)" and "REFRESH
FAILED at <restore time>" although no request was made. A carried,
unrefreshed input now reads "STALE (not refreshed)" / "… CARRIED FORWARD
from an earlier run; this run did not refresh it (restored <time>)". It is
still never FRESH, keeps its as-of, and withholds exactly as before
(`SourceFreshness.carried`; `tests/test_carried_is_not_a_failure.py`). A
real failure still reads REFRESH FAILED.
...
the bootstrap box ticked is refused; a new dispatch comes only after its log
shows no request in the last 24 h (corrected 2026-09-25). Rollback:
...
2. Cloud: Actions → "Weekly dashboard artifact" → Run workflow → tick
   ...
   requested). If a bootstrap fails, do not dispatch another straight away:
   read its log first; a `sleeper_players: FAILED` line means a request was
   made and counts, so wait 24 h after it (step 1). *(Corrected 2026-09-25:
   this said "dispatch a new run".)*
